...
/* emit: mov rax, vr_reg  (vr_reg is the actual x86 encoding from reg_x86[]) */
static void emit_mov_rax_from_vr(x86_emitter_t *e, int vr_enc) {
    /* vr_enc is already the x86 register number (e.g. 10=r10, 3=rbx) */
    rex(e, 1, reg_needs_rex(vr_enc), 0, 0); /* dst=rax(0) */
    e8(e, 0x89);
    e8(e, (uint8_t)(0xC0 | ((vr_enc & 7) << 3) | 0));
}

/* emit: mov vr_reg, rax */
static void emit_mov_vr_from_rax(x86_emitter_t *e, int vr_enc) {
    rex(e, 1, 0, 0, reg_needs_rex(vr_enc)); /* src=rax(0) */
    e8(e, 0x89);
    e8(e, (uint8_t)(0xC0 | (0 << 3) | (vr_enc & 7)));
}
...
#define PATCH_PUSH(pos, lbl) x86_patch_push(&patches, &n_patches, &cap_patches, (pos), (lbl))
    ...
    for (size_t i = 0; i < prog->n; i++) {
        ...
        case MIR_FNEG: {
            ...
            if (sa == 0) {
                ...
                emit_mov_rax_from_vr(&e, sa);  /* mov rax, src_reg */
            } else {
                emit_load_rbp(&e, 0, spill_off(assign, assign_count, &e, in->a));
            }
            ...
            case MIR_DITOF_U: {
                ...
                /* High part: shr rcx, 32; mov edx, ecx; cvtsi2sd xmm0, rdx */
                e8(&e, 0x48); e8(&e, 0xC1); e8(&e, 0xE9); e8(&e, 0x20); /* shr rcx, 32 */
                e8(&e, 0x89); e8(&e, 0xCA); /* mov edx, ecx (zero-extend to rdx) */
                e8(&e, 0xF2); e8(&e, 0x48); e8(&e, 0x0F); e8(&e, 0x2A); e8(&e, 0xC2); /* cvtsi2sd xmm0, rdx */
                /* Load 4294967296.0 (2^32) into xmm2 */
                e8(&e, 0x48); e8(&e, 0xB9);
                ...
                /* Low part: mov ecx, eax; mov edx, ecx; cvtsi2sd xmm1, rdx */
                e8(&e, 0x89); e8(&e, 0xC1); /* mov ecx, eax (low 32 bits) */
                e8(&e, 0x89); e8(&e, 0xCA); /* mov edx, ecx (zero-extend to rdx) */
            ...
            if (in->func_id == 0xFFFF && in->func_name[0]) {
                 ...
                 *   mov vr0, rax (return value) */
                {
                    ...
                    if (sym) {
                        ...
                        /* dlsym failed — check for known functions first */
                        if (strcmp(in->func_name, "isnan") == 0) {
                            /* isnan(double d): check if d is NaN using ucomisd with itself */
                            /* Load d into xmm0 */
        ...
        case MIR_RET: case MIR_FRET: {
            if (x86_in_func_body(prog, i)) {
                ...
                if (sret >= 0 && v0h >= 0) {
                    ...
                    emit_load_rbp(&e, 0, spill_off(assign, assign_count, &e, in->a));
                    if (v0h >= 0) emit_mov_vr_from_rax(&e, v0h);
    ...
    for (size_t i = 0; i < n_patches; i++) {
        ...
        int32_t rel = (int32_t)(t - (pos + 4));   /* rel32: next insn at pos+4 */
