 ...
 *
 * Takes a HolyD AST expression and emits MIR. Everything below this
 ...
 *
 * C11, self-contained. Uses the holyd AST.
 */
#include "holyd_ast.h"
#include "wubu_mir.h"
#include <stdint.h>
...
wubu_vr_t wubu_mir_lower_expr(wubu_mir_prog_t *p, const HDASTNode *n)
{
     ...
     * child (never HD_AST_UNARY — that enum value is legacy). */
    case HD_AST_NEG: {
        wubu_vr_t a = wubu_mir_lower_expr(p, n->child);
        return wubu_mir_unop(p, MIR_NEG, a);
    }
    case HD_AST_NOT: {   /* logical ! : (a == 0) ? 1 : 0 */
        wubu_vr_t a = wubu_mir_lower_expr(p, n->child);
    ...
    case HD_AST_UNARY: {  /* legacy fallback (should not occur) */
        wubu_vr_t a = wubu_mir_lower_expr(p, n->child);
        return wubu_mir_unop(p, MIR_NEG, a);
    }
